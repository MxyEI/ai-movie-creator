...
- **自由板块视频生成误报超时**：视频轮询窗口从 4 分钟（2s×120）延长到 15 分钟（5s×180），与导演板块一致，避免 1080p 的 seedance/sora 等耗时任务被误判超时
- **兼容双层包装的轮询响应**：部分第三方供应商（如 silkroadai）将结果嵌套在 `data.data` 下，新增嵌套感知的视频 URL 提取与状态解析（覆盖外层 / `data` / `data.data` 三层），修复任务实际已完成却取不到 `video_url` 而一直空转直至超时的问题；导演板块同源逻辑一并修复
- **自由板块视频模型被误过滤**：修正视频面板模型过滤逻辑，端点类型与模型名取「或」判定，避免第三方供应商返回非标准 `supported_endpoint_types` 时把明显是视频的模型（如 seedance-2.0-1080）误判为「当前已选模型不可用」

### 📦 其他

- 视频轮询新增进度日志（输出 `progress` 字段）便于排查
- 视频轮询新增连续失败上限（连续 10 次非 200 即快速失败），避免端点已挂仍空转满 15 分钟
